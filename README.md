...
| 技能 | 用途 |
| --- | --- |
| `$wechat-article-workflow` | 全流程编排节点。通过问答补齐必要信息，串联选题/素材收集、内容创作、配图、图片回填排版、用户确认和草稿创建。 |
...
- 从选题、草稿或大纲开始，通过问答串联内容创作、配图、图片回填排版、用户确认和草稿创建。
...
```text
使用 $wechat-article-workflow 从这个选题开始，帮我完成公众号内容创作、配图、图片回填排版和创建草稿。
```
...
```text
使用 $wechat-article-workflow 帮我从选题开始做一篇公众号文章，过程中需要我选择的地方你来问我，按内容创作、配图、图片回填排版、用户确认的顺序推进，最后创建公众号草稿。
```

技能会用少量问答补齐读者、方向、视觉风格、是否生成文内图、是否创建草稿等关键选择。配图完成后会把图片路径回填到 Markdown/HTML 排版稿；默认主线到公众号草稿为止，正式发布必须另行明确确认。
...
技能会优先生成可继续编辑的 Markdown 正文、推荐标题、摘要导语、视觉风格建议和配图 prompt。只有在你要求公众号 HTML 时，才额外生成可复制到编辑器的自包含 HTML；如果你提供已生成图片路径，会把文内图插入对应章节。
